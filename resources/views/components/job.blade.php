...
<a onclick="shell.openExternal('{{ $job->link }}')" href="javascript:;" class="group block px-0 border-b border-gray-200 hover:bg-[#eee] transition-all">
    <div
    ...
    >
        <div class="flex-shrink-0 mb-2">
            @if ($job->company_logo)
                <img class="h-10 w-10 rounded object-contain" src="{{ $job->company_logo }}">
            @else
        ...
        <div class="flex flex-col md:flex-row w-full">
            <div class="flex-1 min-w-0 mb-0 w-full" style="color:{{ $color }};">
                ...
                <p class="text-xs text-gray-500 truncate" style="color:{{ $color }};">{{ $job->creator->name }}</p>
                <p class="text-md font-bold text-gray-900" style="color:{{ $color }};">{{ $job->title }}</p>
                <p class="text-xs text-gray-500" style="color:{{ $color }};">
                    ...
                    @endif
                    @if ($job->salary)
                        <span class="text-gray-500" style="color:{{ $color }};">- {{ $job->salary }}</span>
                    @endif
                </p>
            </div>
            <div class="flex-none text-xs md:flex flex-col md:justify-end text-gray-500 md:px-2" style="color:{{ $color }};">
                <div class="flex justify-between">
                    ...
                    <div class="flex items-center" style="color:{{ $color }};">
                        ...
                        {{ $job->published_at->diffForHumans(['short' => true]) }}
                    </div>
                </div>
